 ...
 */
    ...
    for (
      i = 0;
      i <= number_of_rows;
      i += 1 ///numOfSelectedSlices
    ) {
      if (sliceIsVisible[i] == false) {
        continue;
      }

      sliceAngle[countSelectedSlices] =
        (ciambellaChartData.datasets[0].alloc[i] / allocTotal) * 360;
    ...
    for (i = 0; i < numOfSelectedSlices; i += 1) {
      ...
      for (
        j = 0;
        j < i;
        j += 1 // #### there was a j=1 and j<=i-1 here, double check that is is right!!!!!!! ####
      ) {
        totalAngle += sliceAngle[j];
      }
    ...
    for (i = 0; i < number_of_rows; i += 1) {
      ...
      inner_limit[countSelectedSlices] =
        ciambellaChartData.datasets[0].inner[i];
      countSelectedSlices++;
    }
    ...
    for (i = 0; i < number_of_rows; i += 1) {
      ...
      outer_limit[countSelectedSlices] =
        ciambellaChartData.datasets[0].outer[i];
      countSelectedSlices++;
    }
    ...
    //console.log('r = ' + r);                                //NaN
    var ra = r * 0.6; // *.6;  //ra=r*.6;//*1.83; //1.85;
    ...
    ctx.beginPath();
    ctx.moveTo(
      px[countSelectedSlices - 1] + DataDotRadius,
      py[countSelectedSlices - 1]
    );
    ctx.arc(
      px[countSelectedSlices - 1],
      py[countSelectedSlices - 1],
      DataDotRadius,
      0,
      Math.PI * 2,
    ...
    function drawInnerText() {
      ...
      ctx.fillText(
        "Sustainability",
        cx,
        cy - 33 * (innerRadius / textScalingFactor)
      );
      ctx.fillText(
        "Performance",
        cx,
        cy - 26 * (innerRadius / textScalingFactor)
      );
      ...
      ctx.fillText(
        "strive2thrive.earth",
        cx,
        cy + 21 * (innerRadius / textScalingFactor)
      );
    }
  },
};
  ...
  datasets: [
    {
      ...
      data: [
        50, 60, 65, 22.5, 53, 84, 50, 60, 65, 22.5, 53, 84, 50, 60, 65, 22.5,
        53, 84, 50, 60, 65, 22.5, 53, 84, 50, 60, 65, 22.5, 53, 84, 50, 60, 65,
        22.5, 53, 84,
      ] /* slices values, same as alloc, when putting data in here replicate the data: [] array into the alloc[] array. */,
      alloc: [
        50, 60, 65, 22.5, 53, 84, 50, 60, 65, 22.5, 53, 84, 50, 60, 65, 22.5,
        53, 84, 50, 60, 65, 22.5, 53, 84, 50, 60, 65, 22.5, 53, 84, 50, 60, 65,
        22.5, 53, 84,
      ...
      outer: [
        11, 11, 11, 10, 10, 11, 11, 11, 10, 10, 11, 11, 11, 11, 11, 10, 10, 11,
        11, 11, 10, 10, 11, 11, 11, 11, 11, 11, 10, 10, 11, 11, 11, 10, 10, 11,
        11,
      ],
      impact: [
        0.01, 10, -2, 19, 0.1, 1, 10, -2, 14, 0.1, 1, 10, 19, 10, -2, 14, 0.1,
        1, 19, -2, 14, 0.1, 1, 19, 10, 19, 10, -2, 14, 0.1, 1, 19, -2, 14, 0.1,
        1, 19,
      ],
      topicId: [
        "101",
      ...
      spi: [
        0.001, 1, -0.181, 1.1, 0.001, 0.1, 1, -0.181, 1.4, 0.001, 0.1, 1, 1.9,
        1, -0.181, 1.4, 0.001, 0.1, 1.9, -0.181, 1.4, 0.001, 0.1, 1.9, 1, 1.9,
        1, -0.181, 1.4, 0.001, 0.1, 1.9, -0.181, 1.4, 0.001, 0.1, 1.9,
      ],
      spiScore: 1.358,
